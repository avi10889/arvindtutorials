...
class Qforms_model extends CI_Model
{
    ...
    //////get branch list//////
    function get_branch_list($status = 'all')
    {
        if($status != 'all') $this->db->where("branch_active",$status);
    ...
    function add_branch($data)
    {
        ...
        $sql = $this->db->insert('branch', $data);
        if ($sql) return true;
    }
    ...
    function update_branch($branch_id,$data)
    {
        ...
        {
            $this->db->where('branch_id',$branch_id);
            $sql = $this->db->update('branch',$data);
            if($sql) return true;
        }
    }
    //////Delete Branch//////
    ...
    function update_branch_status($branch_id,$data)
    {
        $this->db->where("branch_id",$branch_id);
        $query = $this->db->get('branch');
        if ($query->num_rows() > 0)
    ...
    //////get Teacher list//////
    function get_teacher_details($status = 'all')
    {
        if($status != 'all') $this->db->where("teacher_active",$status);
    ...
    {
        $sql = $this->db->insert('teachers', $data);
        if ($sql) return true;
    }
    ...
    function update_teacher($teacher_id,$data)
    {
        ...
        {
            $this->db->where('teacher_id',$teacher_id);
            $sql = $this->db->update('teachers',$data);
            if($sql) return true;
        }
    }
    ...
    function update_teacher_status($teacher_id,$data)
    {
        $this->db->where("teacher_id",$teacher_id);
        $query = $this->db->get('teachers');
        if ($query->num_rows() > 0)
